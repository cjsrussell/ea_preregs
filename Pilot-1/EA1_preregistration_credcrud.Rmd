...
This preregistration is part of a number of preregistrations for the Explaining Atheism project. Our other registrations can be found on the OSF [here](https://osf.io/df6m8/), with copies on our FigShare, and on my GitHub.
...
Credibility Enhancing Displays are costly displays made by a model, particularly those that reflect a statement made by that model. Displays that show a model walking the walk, or practicing what they preach, if you will. For example if a model says a blue mushroom is toxic and then avoids eating it even when hungry, or priestly celibacy would be CREDS. A number of studies have shown historical CRED exposure to predict religiosity (CITES). The present project looks to build upon this, and also update the measures for cross-cultural comparison.

Conversely CRUDs are behaviours which undermine credibility. They are behaviours where a model might engage in behaviours which contradict their stated belief, such as breaking celibacy, or eating haram foods.
...
The explanatory variables of primary interest, CRUDs and CREDs are part of a survey with the following explanatory variables:
...
Similarly the response variables of interest are part of a wide range of response variables that are collected, see the codebook for other variables:
...
Our explanatory variables are sca
